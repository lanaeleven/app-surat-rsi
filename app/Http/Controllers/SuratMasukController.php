...
use App\Services\DisposisiTerusanResolver;
...
class SuratMasukController extends Controller
{
    ...
    public function disposisi(SuratMasuk $suratMasuk, DisposisiTerusanResolver $resolver) {
        ...
        // }
            ...
            $terusan = $resolver->resolve($user);

            if (is_string($terusan)) {
                return back()->with('error', $terusan);
            }
            ...
            if (DistribusiSurat::where('idSuratMasuk', '=', $suratMasuk->id)->exists()) {
                ...
                if ((! Gate::allows('disposisi-surat', $cekDS) || $cekDS->status == "Diarsipkan") && $user->id != 1) {
                    abort(403);
                }
            }
            ...
            if ($suratMasuk->status == "Belum Diteruskan" && $user->id != 1) {
                abort(403);
            }
